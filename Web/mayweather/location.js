				...
				navigator.geolocation.getCurrentPosition(function(p) {
			        ...
			        $.ajax({
			        	...
			        	success: function(data){
                            ...
                            var itemData = data.response.body.items.item;
                            
                           // 카테고리와 값만 받아오기
                           for(var i=0; i < itemData.length; i++){
                                var fcstValue = itemData.fcstValue;
                                var fcstDate = itemData.fcstDate;
                                var fcstTime = itemData.fcstTime;

                                category = JSON.stringify(itemData.category);

                                $('.weather_now').remove(html);
                                
                                var html = '<div class="weather_now">'
                                            + '<table>'
                                            + '<tr><td class="font4">' +  itemObj['category'].PTY + '</td></tr>'
                                            + '<tr><td class="font2">'+itemObj['category'].T1H+'</td></tr>'
                                            + '<tr><td class="font5">+최고: 7° 최저: -3°+</td></tr>'
                                            + '</table>'
                                            + '</div>';
                                
                                $('.weather_now').append(html);  
