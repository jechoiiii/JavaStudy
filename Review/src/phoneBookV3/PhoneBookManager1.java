...
*/		 
	...
	PhoneInfo1[] phoneBook;	// 정보 저장을 위한 배열 선언 // PhoneInfo타입 [] 배열의 이름;
	int cnt; // 저장된 정보의 개수 
	
//	Scanner sc = new Scanner(System.in);
	
	// 배열 초기화 - 생성자
	PhoneBookManager1(int size){	// 생성자는 클래스 이름과 동일, int 타입의 
		// 배열의 저장공간을 생성
		phoneBook = new PhoneInfo1[size];	// 배열이름 = new PhoneInfo타입[배열의크기]
	}
	PhoneBookManager1() {	// 매개 변수x, 생성자 오버로딩
		// 배열의 저장공간을 100으로 설정
		this(100);
	}
	...
	void insertMember() {	// 반환x void
		...
		PhoneInfo1 info = null;	// 타입 참조변수 = 기본값;
		
		// 인스턴스 생성 - 분기
		if(birthday != null && birthday.length()>0) {
			info = new PhoneInfo1(name, pNum, birthday);
		} else {
			info = new PhoneInfo1(name, pNum);
		}
