...
// 2020.10.08 : 생성자 정의, 변수의 초기화 목적
	...
	// FruitSeller seller -> FruitSeller 클래스로 만든 객체의 주소를 저장하는 변수 (FruitSeller클래스의 타입에 맞는 변수 타입만 가능)
	void buyApple(FruitSeller seller, int money) {			
		
		// 구매할 사과의 개수를 구하기
		int num = seller.saleApple(money);					// seller.myMoney는 판매자의 수익 금액 가져오는 것
		// (Buyer) money -> (Seller) money -> return으로 메서드 종료 -> (Seller)의 num (Buyer)의 num에 대입
		// 나의 사과의 개수를 증가
		numOfApple += num;
		// 나의 보유 금액을 감소
		myMoney -= money;
