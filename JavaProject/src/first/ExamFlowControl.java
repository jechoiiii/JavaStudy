...
//		문제1. 아래 예제는 두 개의 if문을 사용하고 있다. 한 개의 if 문만 사용하는 방식으로 변경해보자.
		...
		// 중첩된 if는 하나의 if로 합칠 수 있다!	*리프트링(고도화): 코드개선, 유지보수 위해 안 쓰는 코드를 없애고 반복되는 코드 합치기 		
		if(num>0 && num%2==0) {
			System.out.println("양수이면서 짝수");
		}
...
//		문제 3. CondOp.java를 조건연산자(3항 연산자)를 사용하지 않고, if~else를 사용하는 형태로 변경해 보자.
		System.out.println("문제 3. ===============================================");
		
		int num1=50, num2=100;
		int big, diff;
		//big = (num1>num2)? num1:num2;				// *삼항연산자 이용해서 사과바구니 구하는 문제 해보기 
	/*	if(num1>num2) {
			big = num1;
		} else {
			big = num2;
...
//		문제 4. SwitchBreak.java를 switch문이 아닌, if~else를 사용하는 형태로 변경해 보자.
		...
		int n=3;	// n의 데이터는 사용자나 DB에서 받아올 수 있다.
	/*	switch(n) {
			case 1:
