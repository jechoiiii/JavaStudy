	...
	public void memberLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if(member != null) {
				...
				// getSession() -> Session 이 있으면 있는 session 을 반환, 없으면 새로운 session 객체를 생성해서 반환 
				// getSession(false) -> Session 이 있으면 있는 session을 반환, 없으면 null 을 반환 
				// getSession(true) -> 새로운 session 객체를 생성해서 반환
